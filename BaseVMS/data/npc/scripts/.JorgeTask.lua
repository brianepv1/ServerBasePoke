...
local task = {
["Jorge"] = {{"Charizard", 120}, {"Charmeleon", 65}},   -- 120 Charizards, e 65 Charmeleons
}
        ...
        if msgcontains(msg, 'task') or msgcontains(msg, 'Task') then
           if isMyTaskComplete(cid, getNpcCid()) then
              ...
              doPlayerAddExperience(cid, 1000000)  --premio   --Quase 2 leveis no nivel 101
              doPlayerAddItem(cid, 2160, 10)
              local sto = getMyTaskSto(cid, getNpcCid())
              setPlayerStorageValue(cid, sto, -1)
              setPlayerStorageValue(cid, 181610, 1) --storage da outfit
              talkState[talkUser] = 0
              return true
           else
              selfSay("Then, is a simple task, i want to you kill {120 Charizards, 65 Charmeleons,}, you accept the task?", cid)

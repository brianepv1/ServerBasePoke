...
function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg, 'help') or msgcontains(msg, 'quest') then
        ...
        selfSay("Ola, meu amigo, voce pode me trazer: 100 Fragments of Mega Box? Eu o recompensarei muito bem!", cid)
        talkState[talkUser] = 1
        return true
    elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
        if getPlayerItemCount(cid, need[1].id) < need[1].qt then
            selfSay("Voce nao possui todos os 100 fragmentos necessarios.", cid)
            selfSay("Lembre-se, voce precisa me trazer 100 fragmentos da mega box que dropa na OutLand...", cid)
            talkState[talkUser] = 0
            return true
        end
        for i = 1, #need do

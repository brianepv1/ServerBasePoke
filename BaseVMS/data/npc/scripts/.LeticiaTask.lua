...
local task = {
["Leticia"] = {{"Azumarill", 80}, {"Tentracruel", 100}},   -- 1
}
        ...
        if msgcontains(msg, 'task') or msgcontains(msg, 'tarefa') then
           if isMyTaskComplete(cid, getNpcCid()) then
              ...
              setPlayerStorageValue(cid, 181655, 1) --storage da outfit
              talkState[talkUser] = 0
              return true
           else
              selfSay("Então, é uma tarefa simples, eu quero que você mate {80 Azumarills,100 Tentacruels}, você aceita a tarefa?", cid)

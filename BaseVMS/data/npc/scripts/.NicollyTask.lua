...
local task = {
["Nicolly"] = {{"Gengar", 70}, {"Haunter", 55}, {"Gastly", 35}},              -- 70 Gengar         55 Haunter  35 Gastly
}
        ...
        if msgcontains(msg, 'task') or msgcontains(msg, 'Task') then
           if isMyTaskComplete(cid, getNpcCid()) then
              ...
              doPlayerAddExperience(cid, 1200000)  --premio
			  doPlayerAddItem(cid, 2160, 10)
              local sto = getMyTaskSto(cid, getNpcCid())
              setPlayerStorageValue(cid, sto, -1)
              setPlayerStorageValue(cid, 181607, 1) --storage da outfit
              talkState[talkUser] = 0
              return true
           else
              selfSay("Then, is a simple task, i want to you kill {70 Gengars, 55 Haunter, 35 Gastly,}, you accept the task?", cid)

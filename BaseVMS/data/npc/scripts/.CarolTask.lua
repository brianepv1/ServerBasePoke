...
local task = {
["Carol"] = {{"Blastoise", 80}, {"Kingler", 100}},   -- 1
}
        ...
        if msgcontains(msg, 'task') or msgcontains(msg, 'Task') then
           if isMyTaskComplete(cid, getNpcCid()) then
              ...
              setPlayerStorageValue(cid, 181645, 1) --storage da outfit
              talkState[talkUser] = 0
              return true
           else
              selfSay("Then, is a simple task, i want to you kill {80 Blastoises,100 Kinglers}, you accept the task?", cid)

...
local task = {
["Gabriela"] = {{"Ampharos", 80}, {"Electrode", 100}},   -- 1
}
        ...
        if msgcontains(msg, 'task') or msgcontains(msg, 'tarefa') then
           if isMyTaskComplete(cid, getNpcCid()) then
              ...
              setPlayerStorageValue(cid, 181654, 1) --storage da outfit
              talkState[talkUser] = 0
              return true
           else
              selfSay("Então, é uma tarefa simples, eu quero que você mate {80 Ampharos,100 Electrodes}, Você aceita a tarefa?", cid)

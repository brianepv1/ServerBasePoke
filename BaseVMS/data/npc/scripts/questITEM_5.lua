...
function creatureSayCallback(cid, type, msg)
   ...
   ---------
   local need = {
      {id = 12194, qt = 250}, --future orb
	  {id = 12193, qt = 50},  -- psychic
   }
   local rewards = {
      {id = 2160, qt = 1}, --money
	  {id = 2392, qt = 100}, -- ultra ball
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Hello my friend, can you bring to me: 250 future orb and 50 twisted spoons? I will reward you!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      if getPlayerItemCount(cid, need[1].id) < need[1].qt or getPlayerItemCount(cid, need[2].id) < need[2].qt then
         ...
         selfSay("Remember, you need to bring to me 250 future orb and 50 twisted spoons...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do

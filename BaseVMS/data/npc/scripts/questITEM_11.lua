...
function creatureSayCallback(cid, type, msg)
   ...
   ---------
   local need = {
	  {id = 12171, qt = 100},  -- pot of moss bug
   }
   local rewards = {
      {id = 11448, qt = 5},
   }
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Olá meu amigo, você pode trazer para mim: 100 pot of moss bug? Vou recompensá-lo!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      if getPlayerItemCount(cid, need[1].id) < need[1].qt then
         ...
         selfSay("Lembre-se, você precisa trazer para mim 100 pot of moss bug...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do

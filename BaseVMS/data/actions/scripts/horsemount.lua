...
local config = {
    outfitMale = 5012, -- Outfit male
    outfitFemale = 5066, -- Outfit female
    storageValue = 32010, -- Storage Para a bike
}
...
function onUse(cid, item, itemEx, fromPosition, toPosition)
    ...
    if ctrl < 1 then
        ...
        doPlayerSendCancel(cid, "Você não pode usar montaria e robo ao mesmo tempo.")
		return false
end

		if getPlayerStorageValue(cid, 92002) >= 1 then
        doPlayerSendCancel(cid, "Você não pode usar mount e robo ao mesmo tempo.")
		return false
end
...
if getPlayerStorageValue(cid, 19000) == 1 then
        doPlayerSendCancel(cid, "Você não pode usar mount e correr ao mesmo tempo.")
		return false
end
        ...
        doPlayerSendCancel(cid, "Você não pode usar mount enquanto está no Fly/Hide.")
    end
return true
end
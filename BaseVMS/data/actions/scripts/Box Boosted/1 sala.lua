local posis = {{'Volcanic', {
    x = 1237,
    y = 616,
    z = 8
}}, -- nome de cada clan e a pos aonde o player desse clan tem q tar...
{'Seavell', {
    x = 1237,
    y = 618,
    z = 8
...
local toPosi = {
    ...
    y = 551,
    z = 9
} -- pos pra onde os players serao teleportados
    ...
    for _, array in ipairs(posis) do
        ...
        if not isPlayer(p) or getPlayerClanName(p) ~= array[1] and getPlayerLevel(cid) <= 120 then
            doPlayerSendTextMessage(cid, 27,
                "Esta faltando um membro de clan ou alguem n�o possui level necess�rio!")
            return true
        end
    end

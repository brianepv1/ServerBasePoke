        ...
        if isChannelTv(channelId) then
            ...
            if isCreature(owner) then
                ...
                if owner ~= cid then
                    ...
                    doPlayerSendChannelMessage(owner, "TV Channel",
                        getCreatureName(cid) .. ' is now watching your channel (currently ' .. #users - (1) .. ' player' ..
                            plural .. ' watching this channel).', 15, channelId)
    ...
    if channelId >= 100 and channelId <= 10000 then
        ...
        if isChannelTv(channelId) then
            ...
            if owner ~= cid and getCreatureOutfit(cid).lookType == 814 then
                ...
                doPlayerSendChannelMessage(owner, "TV Channel",
                    getCreatureName(cid) .. ' is not watching your channel anymore (currently ' .. #users - (1) ..
                        ' player' .. plural .. ' watching this channel).', 15, channelId)
...
function onWalk(cid, fromPosition, toPosition)
    ...
    if getPlayerStorageValue(cid, 99284) <= 0 then
        return true
    end
...
local permited = {"a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t",
    ...
    if item.itemid == 12330 then
        ...
        if string.len(newText) <= 0 then
            ...
            doPlayerSendCancel(cid,
                "Your channel is going to be shown as \"" .. getCreatureName(cid) .. "'s TV Channel\".")

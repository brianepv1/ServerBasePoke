...
local stg = conds["Miss"]
    ...
    if  isStaff(cid) then return true end
...
function isStaff(cid)
	if isPlayer(cid) and getPlayerGroupId(cid) >= 4 then return true end
end
...
local pkmns = {"articuno","zapdos","moltres","mewtwo","mew","raikou","entei","suicune","lugia","ho-oh","celebi","regirock","registeel","latios","latias","kyogre","groudon","rayquaza","deoxys","uxie","mesprit","azelf","dialga", "palkia","heatran","regigigas","cresselia","phione","Shaymin","arceus","victini","cobalion","terrakion","Virizion","tornadus","Thundurus","Reshiram","zekrom","landorus","kyurem", "Keldeo","Meloetta","Genesect"} --Pokémons imunes a status negativos
if not isSummon(ret.id) and isInArray(pkmns, getCreatureName(ret.id)) then
	return true
end
